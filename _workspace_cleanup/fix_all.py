...
def fix_file(path):
    ...
    text = re.sub(r'if p == 0i64 \{', 'if (p == 0i64) {', text)
    text = re.sub(r'if fp == 0i64 \{', 'if (fp == 0i64) {', text)
    text = re.sub(r'if size <= 0i64 \{', 'if (size <= 0i64) {', text)
    text = re.sub(r'if size <= 4096i64 \{', 'if (size <= 4096i64) {', text)
    text = re.sub(r'if buf == 0i64 \{', 'if (buf == 0i64) {', text)
    text = re.sub(r'if f.buf_pos == 0i64 \{', 'if (f.buf_pos == 0i64) {', text)
    text = re.sub(r'while remaining > 0i64 \{', 'while (remaining > 0i64) {', text)
    text = re.sub(r'if r.is_error \{', 'if (r.is_error) {', text)
    text = re.sub(r'if r.value == 0i64 \{', 'if (r.value == 0i64) {', text)
    text = re.sub(r'if e == EINTR \{', 'if (e == EINTR) {', text)
    text = re.sub(r'if unread <= 0i64 \{', 'if (unread <= 0i64) {', text)
    text = re.sub(r'if f.buf_mode == _IONBF \|\| f.buf == 0i64 \|\| f.buf_cap == 0i64 \{', 'if (f.buf_mode == _IONBF || f.buf == 0i64 || f.buf_cap == 0i64) {', text)
    text = re.sub(r'while true \{', 'while (true) {', text)
    text = re.sub(r'if m\[0\] == 0u8 \{', 'if (m[0] == 0u8) {', text)
    text = re.sub(r'while m\[i\] != 0u8 \{', 'while (m[i] != 0u8) {', text)
    text = re.sub(r'if m\[i\] == 43u8 \{', 'if (m[i] == 43u8) {', text)
    text = re.sub(r'if m\[i\] == 98u8 \{', 'if (m[i] == 98u8) {', text)
    text = re.sub(r'if has_b \{', 'if (has_b) {', text)
    text = re.sub(r'if has_plus \{', 'if (has_plus) {', text)
    text = re.sub(r'if first == 114u8 \{', 'if (first == 114u8) {', text)
    text = re.sub(r'if first == 119u8 \{', 'if (first == 119u8) {', text)
    text = re.sub(r'if first == 97u8 \{', 'if (first == 97u8) {', text)
    text = re.sub(r'if parse_ok != 0i64 \{', 'if (parse_ok != 0i64) {', text)
    text = re.sub(r'if parse_res < 0i64 \{', 'if (parse_res < 0i64) {', text)
    text = re.sub(r'if f.fd < 0i64 \{', 'if (f.fd < 0i64) {', text)
    text = re.sub(r'if f == 0i64 \{', 'if (f == 0i64) {', text)

    # 4. Void to NIL in file.npk
    if 'file.npk' in path:
    ...
    if (first == 114u8) {  // 'r'
        ...
        oflags = has_plus ? O_RDWR : O_RDONLY;
        if (has_plus) { fmode = fmode | FILE_MODE_WRITE; }
    ...
    int64:open_flags = parse_res >> 32i64;
    int64:file_mode = parse_res & 0xFFFFFFFFi64;'''
        text = re.sub(r'    stack int64:file_mode_out\[1\];.*?if parse_ok != 0i64 \{\n        fclose\(fp\);\n        drop libn_errno_set\(EINVAL\);\n        pass 0i64;\n    \}', repl_parse2, text, flags=re.DOTALL)
